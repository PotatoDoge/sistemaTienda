...
class ShowInventory : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val returnInv = findViewById<ImageButton>(R.id.returnShowInv)
        val filtrar = findViewById<Button>(R.id.filtrarTablaInv)
        table = findViewById(R.id.inventoryTable)

        mostrarInv("http://charlyffs.mywire.org:9000/llenar_tabla_inv.php","Todo")
    ...
    private fun selectCategories(){
        ...
        builder.setPositiveButton("Ok", DialogInterface.OnClickListener {
                dialog,id ->
            mostrarInv("http://charlyffs.mywire.org:9000/llenar_tabla_inv.php",catSelectedValue)
        }).setCancelable(false)
    ...
    private fun mostrarInv(URL:String, filtro:String){
                ...
                if(response.isNotEmpty()){
                    try {
                        ...
                        for(i in 0 until jsonArray.length()){
                            ...
                            val pu = newFila.findViewById<View>(R.id.nombrePrd) as TextView
                            val ex = newFila.findViewById<View>(R.id.descPrd) as TextView
                            val precio = "$" + obj.getString("precioUnitario")
                            id.text = obj.getString("idProducto") // cambiar por valor de id del jsonObject
                            pu.text = precio
                            ex.text = obj.getString("existencia")
